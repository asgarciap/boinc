            ...
            for(int i=0; i<nwus; i++) {
                ...
                if(bavp) {
                    ...
                    if((int) avg_turnaround_time() > 0) {
                        if(ewd < (int) avg_turnaround_time())
                            ewd = ceil(avg_turnaround_time());
                    }else if(sreq->other_results.size() > 0){
                        log_messages.printf(MSG_NORMAL,"[mge_sched] [seas]sched] [HOST#%ld] Waiting for host to report its first finished job to compute estimated turnaround\n",sreq->hostid);
                        break;
                    }
                    ...
                    if( tot_busy > newuptime && !sreq->host.on_ac_power && !sreq->host.on_usb_power) {
                        ...
                        if(rr == 0) {
                            tewd += ewd;
                            //sent only one result to get the avg turnaround estimation
