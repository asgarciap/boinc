...
int add_result_to_reply(
    ...
    if(config.mge_replication) {
        //if we are sending an unreplicated job and we are using
        //mge-replication. calculate the number of replicas to generate
        if(wu.target_nresults == 1 && app->target_nresults > 1) {
            int nresults = 1;
            int min_quorum = 1;
            calc_workunit_replicas(g_request, &wu, nresults, min_quorum, app->target_nresults);
            if(nresults > app->target_nresults) nresults = app->target_nresults;
            if(nresults > 1) {
